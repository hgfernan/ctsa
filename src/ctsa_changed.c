...
sarima_object sarima_init(int p, int d, int q,int s, int P,int D,int Q, int N) {
...
#else /* ! OLD_IMPL */
	obj = (sarima_object)calloc(1, sizeof(struct sarima_set));
	...
	obj->phi = (double*)malloc(sizeof(double)* p);
	obj->theta = (double*)malloc(sizeof(double)* q);
	obj->PHI = (double*)malloc(sizeof(double)* P);
	obj->THETA = (double*)malloc(sizeof(double)* Q);
	
	obj->res = (double*)malloc(sizeof(double) * obj->N);
	
	// Variance-Covariance Matrix Of length lvcov
	obj->vcov = (double*)calloc(obj->lvcov, sizeof (double)); 
...
#else /* ! OLD_IMPL */
		obj->retval = as154_seas(inp, obj->N, obj->optmethod, obj->p, obj->d, obj->q, obj->s, obj->P, obj->D, obj->Q, 
			obj->phi, obj->theta, obj->PHI, obj->THETA, &obj->mean, &obj->var, &obj->loglik, obj->vcov, obj->cssml);
