...
\begin{enumerate}
    \item \label{itm:models}
        The most central table is {\tt models} that contains
    ...
    \item \label{itm:codes}
        ...
        its source code filename would be ``{\tt 0015.c}''. But
        there's a lot of parameters to consider when dealing with
        source code files, so another naming scheme will be used.

        If the code uses the $AR$ model implementation of
        {\tt ctsa}, uses the 1st group of parameters, and the
        {\tt datafile\_id} of $23$, the extended name of its
        source code will be ``{\tt ctsa\_ar\_p0001\_d0023.c}''.

        In a typical Linux setup the executable name will be
        ``{\tt ctsa\_ar\_p0001\_d0023}'', and in a Microsoft
        Windows setup it will be
        ``{\tt ctsa\_ar\_p0001\_d0023.exe}''. However, the
        Windows version of the software will be postponed for
        a while.

        The library name will precede the model name because
        there's much more commonality between the creation of
        executable files of a library than there are between
        different implementations of the same statistical model
...
\begin{enumerate}
    \item {\bf Data transcription}: The data series files
        available are adapted to a simple format CSV format that
        contains only an index column, and a value column.

        The table {\tt file\_db.csv} contains a mapping to the
        original data and information on the column that was
        used. For original data files with several value columns,
        several files in this simple format can be generated.

        This phase is partly manual, in the configuration of the
        {\tt file\_db.csv} file for each original datafile, and
        partly automatic, since the program {\tt cvt\_test.py}
        will convert automatically the data files to this test
        format, provided they're configured in {\tt file\_db.csv}

    \item \label{itm:creation_step}
        {\bf Template creation}: This step is totally manual,
        and contains three substeps:

        \begin{enumerate}
            \item \label{itm:ctsa}
                The existing {\tt ctsa} test files are used to
                create {\tt C} source code templates as Python
                strings that will have the parameter information
                filled in.

                From the {\tt ctsa} source code templates it is
                extracted the statistical parameters that are
                named {\em arguments} in this context: they are
                the theoretical and universal part of the
                configuration used to be run the {\tt ctsa} code.
                The pratical configuration, that includes parts
                specific to {\tt ctsa} code is named
                {\em paramters};

            \item \label{itm:Python}
                One or more Python libraries are identified with
                exactly the one or more algorithms that are the
                same as the ones in {\tt ctsa}.

                The chosen library or libraries should accept the
                {\em arguments} (see the item \ref{itm:ctsa}),
                but there's liberty to adjust the {\em arguments}
                to the library using a group of
                {\em parameters} specific to it;

                Currently, only {\tt statsmodels} is being
                considered for generic algorithms, and
                {\tt pmdarima} for automated hyperparameter
                tuning. But other libraries are available. In any
                case, the data structure defines templates by
                library, not by language. Therefore, any number
                of libraries with different languages can be used,
                provided they have the same algorithm as
                {\tt ctsa}.

                In this substep, study and manual programming will
                lead to templates and parameters that generate
                results equivalent to the ones of {\tt ctsa};

            \item \label{itm:R}
                One or more R libraries are identified with
                exactly the one or more algorithms that are the
                same as the ones in {\tt ctsa}.
                ...
                Currently, only Hyndman's {\tt forecast} is being
                considered. Under the covers, {\tt forecast} uses
                the package {\tt stats} for generic algorithms,
                and implements an automated hyperparameter tuning
                named {\tt auto.arima}. Other R libraries are
                available, like {\tt fable} by the same research
                group of Monash University.

                In any case, the data structure defines templates
                by library, not by language. Therefore, any number
                of libraries with different languages can be used,
                provided they have the same algorithm as
                {\tt ctsa};
    ...
    \item \label{itm:generation}
        {\bf Code generation}: This step is totally automatic, and
        from the data files, the templates and parameters, codes
        are generated.

        They will be compiled or simply installed in the next
        step (item \ref{itm:compilation});

    \item \label{itm:compilation}
        {\bf Compilation or installation}: In this step, the
        {\tt C} codes are compiled, and transferred to the
        folder of their base libraries; the Python and R codes
        are just installed in the folder of their base libraries,
        but the installation makes sure that the setup has the
        expected libraries.

        In more detail: the {\tt C} code is compiled against the
        {\tt ctsa} library only and some care was taken to not use
        other libraries. But the Python and R codes should
        be guaranteed to have installed all the packages or
        modules they need.

        Any compilation failure, or absence of the needed package
        will generate a failure message in the compilation log.

    \item \label{itm:execution}
        {\bf Execution}: This phase causes the results to be
        generated. For each execution the master database is
        updated, the code output (containing a statistical summary
        or a failure message) is redirected to a file named
        according to the code and execution, and a new entry is
        generated in the log of executions;

    \item \label{itm:comparison}
        {\bf Comparison and test}: This phase will give tools to
        finally test the results of the executions. This allows
        different versions of the same library to be compared,
        as well as the results of the execution of codes using
        two different libraries. In any case, a valid comparison
        will demand the same statistical model, the same
        {\em arguments}, defined in item \ref{itm:ctsa}. To have
        a performance comparison, the same hardware should be
        considere. That's provided in the system description of
        each execution, contained in the field {\tt sys\_dec} of
        the table {\tt execs}, that's defined in the item
        \ref{itm:execs} of the section \nameref{sec:tables}.

    \item \label{itm:more_arguments}
        {\bf Additional step -- More arguments}:
        Many of the tests in the current distribution of
        {\tt ctsa} use just a single parameter for model. But
        obviously the testing will be much more complete if a
        larger number of parameters, in different ranges, are
        used. This module will be used to improve such quantity.

        As of now there's not a policy of adding more parameter
        values, but the data structure available in table
        {\tt paramlims} seems to be flexible enough to accomodate
        any policy:  only the upper and lower limits are
        considered.
